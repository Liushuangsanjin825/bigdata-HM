...
- 仓库名称：bigdata-HM
- 当前阶段：第二阶段进行中（期末工程交付 + Kaggle 调优，进度 ~50%）
...
- ✅ 已覆盖：期中考核（20%）
- ✅ 已覆盖：期末工程交付基础（LightGBM baseline + 消融/SHAP 分析）
- 🔄 进行中：Kaggle 打榜优化（LightGBM 候选增强 + 超参调优）
- ❌ 未覆盖：答辩与贡献度（15%）
...
7. 轻度 Optuna 超参优化：使用 20-25 trials 搜索 learning_rate、num_leaves、max_depth、feature_fraction、bagging_fraction、lambda_l1/l2。
